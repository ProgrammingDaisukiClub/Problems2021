...
void maze_gen(string fname, int cases, int minn, int maxn, int minq, int maxq);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 10; ++i)
	...
	maze_gen("maze.in", LARGE_T, LARGE_MIN_N, LARGE_MAX_N, LARGE_MIN_Q, LARGE_MAX_Q);
  return 0;
}
...
void random(string fname, int cases, int minn, int maxn, int minq, int maxq) {
  ...
  while (cases--) {
    ...
    set<int> st;
    while (st.size() < q)
      st.insert(rnd.next(0, m - 1));
    for (auto p : st)
      of << v[p].first + 1 << " " << v[p].second + 1 << endl;
  }
  of.close();
}
...
void maze_gen(string fname, int cases, int minn, int maxn, int minq, int maxq) {
  ...
  while (cases--) {
		...
		{
			for(int i = 0; i < N; i++) {
				for(int j = 0; j < N; j++) if(G[i][j] == '.') kouho.emplace_back(i, j);
			}
			sz = int(kouho.size());
		...
		of << Q << endl;
		for(int i : se) of << kouho[i].first+1 << " " << kouho[i].second+1 << endl;
  }
	of.close();
}