...
def build_statement(dir_name):
    ...
    constraints_h = open_and_read_file(
        os.path.join(dir_name, 'tests', CONSTRANITS_FILENAME))
    ...
    while True:
        ...
        statement = statement[:match.start()] + result_str + \
            statement[match.end():]
    ...
    if match:
        input_str = open_and_read_file(os.path.join(
            dir_name, 'rime-out', 'tests', SAMPLE_INPUT_FILENAME))
        output_str = open_and_read_file(os.path.join(
            dir_name, 'rime-out', 'tests', SAMPLE_OUTPUT_FILENAME))
        sample_str = '''# 入力例
```
{}```
...
{}```'''.format(input_str, output_str)
        statement = statement[:match.start()] + sample_str + \
            statement[match.end():]
    ...
    for problem in data['problems']:
        ...
        for data_set in problem['data_sets']:
            test_dir = os.path.join(
                dir_name, 'rime-out', 'tests', data_set['label'].lower())
        ...
        # output to std
        data_set_str = ["{} ({})".format(s['label'], s['score'])
                        for s in problem['data_sets']]
